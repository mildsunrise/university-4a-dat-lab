...
addQuestion quest conn = do
    execute conn "INSERT INTO questions (theme,user,posted,title,body) VALUES (?,?,?,?,?)" quest
    lastInsertRowId conn

updateQuestion :: QuestionId -> Question -> ForumDb -> IO ()
updateQuestion qid quest conn =
    execute conn "UPDATE questions SET theme=?,user=?,posted=?,title=?,body=? WHERE id = ?" (quest :. Only qid)
...
addAnswer ans conn = do
    execute conn "INSERT INTO answers (question,user,posted,body) VALUES (?,?,?,?)" ans
    lastInsertRowId conn

updateAnswer :: AnswerId -> Answer -> ForumDb -> IO ()
updateAnswer aid ans conn =
    execute conn "UPDATE answers SET question=?,user=?,posted=?,body=? WHERE id = ?" (ans :. Only aid)
